...
invid = cv.CaptureFromFile("../ONI_TestBench/testDepthFrame.jpg")
cv.ShowImage("winname", invid)
...
# lena = numpy.fromstring(cv.LoadImage("lena.jpg", 0).tostring(), numpy.uint8) / 255.0
...
#ChessBoard 8 x 6
#images = [cv.LoadImageM("right01.jpg", cv.CV_LOAD_IMAGE_COLOR),cv.LoadImageM("right05.jpg",  cv.CV_LOAD_IMAGE_COLOR),cv.LoadImageM("left01.jpg",  cv.CV_LOAD_IMAGE_COLOR),cv.LoadImageM("left09.jpg",  cv.CV_LOAD_IMAGE_COLOR)]

#ChessBoard 13 x 11
#images = [cv.LoadImageM("test01.jpg", cv.CV_LOAD_IMAGE_COLOR),cv.LoadImageM("test02.jpg",  cv.CV_LOAD_IMAGE_COLOR),cv.LoadImageM("test03.jpg",  cv.CV_LOAD_IMAGE_COLOR),cv.LoadImageM("test04.jpg",  cv.CV_LOAD_IMAGE_COLOR)]


#ChessBoard 10 x 7
#images = [cv.LoadImageM("New01.jpg", cv.CV_LOAD_IMAGE_COLOR),cv.LoadImageM("New02.jpg",  cv.CV_LOAD_IMAGE_COLOR),cv.LoadImageM("New03.jpg",  cv.CV_LOAD_IMAGE_COLOR),cv.LoadImageM("New04.jpg",  cv.CV_LOAD_IMAGE_COLOR)]
#images = [cv.LoadImageM("KinectTest01.jpg", cv.CV_LOAD_IMAGE_COLOR),cv.LoadImageM("KinectTest02.jpg",  cv.CV_LOAD_IMAGE_COLOR),cv.LoadImageM("KinectTest03.jpg",  cv.CV_LOAD_IMAGE_COLOR),cv.LoadImageM("KinectTest04.jpg",  cv.CV_LOAD_IMAGE_COLOR),cv.LoadImageM("KinectTest05.jpg",  cv.CV_LOAD_IMAGE_COLOR)]
#images = [cv.LoadImageM("IR1.png", cv.CV_LOAD_IMAGE_COLOR),cv.LoadImageM("IR2.png",  cv.CV_LOAD_IMAGE_COLOR),cv.LoadImageM("IR3.png",  cv.CV_LOAD_IMAGE_COLOR),cv.LoadImageM("IR4.png",  cv.CV_LOAD_IMAGE_COLOR),cv.LoadImageM("KinectTest05.jpg",  cv.CV_LOAD_IMAGE_COLOR)]




images1 = [cv.LoadImageM("color0.png", cv.CV_LOAD_IMAGE_COLOR),
          cv.LoadImageM("color1.png", cv.CV_LOAD_IMAGE_COLOR),
          cv.LoadImageM("color2.png", cv.CV_LOAD_IMAGE_COLOR),
          cv.LoadImageM("color3.png", cv.CV_LOAD_IMAGE_COLOR),
          cv.LoadImageM("color4.png", cv.CV_LOAD_IMAGE_COLOR),
          cv.LoadImageM("color5.png", cv.CV_LOAD_IMAGE_COLOR),
          cv.LoadImageM("color6.png", cv.CV_LOAD_IMAGE_COLOR),
          cv.LoadImageM("color7.png", cv.CV_LOAD_IMAGE_COLOR),
          cv.LoadImageM("color8.png", cv.CV_LOAD_IMAGE_COLOR),
          cv.LoadImageM("color9.png", cv.CV_LOAD_IMAGE_COLOR),
          cv.LoadImageM("color10.png", cv.CV_LOAD_IMAGE_COLOR),
          cv.LoadImageM("color11.png", cv.CV_LOAD_IMAGE_COLOR)]

images2 = [cv.LoadImageM("intensity0.png", cv.CV_LOAD_IMAGE_COLOR),
          cv.LoadImageM("intensity1.png", cv.CV_LOAD_IMAGE_COLOR),
          cv.LoadImageM("intensity2.png", cv.CV_LOAD_IMAGE_COLOR),
          cv.LoadImageM("intensity3.png", cv.CV_LOAD_IMAGE_COLOR),
          cv.LoadImageM("intensity4.png", cv.CV_LOAD_IMAGE_COLOR),
          cv.LoadImageM("intensity5.png", cv.CV_LOAD_IMAGE_COLOR),
          cv.LoadImageM("intensity6.png", cv.CV_LOAD_IMAGE_COLOR),
          cv.LoadImageM("intensity7.png", cv.CV_LOAD_IMAGE_COLOR),
          cv.LoadImageM("intensity8.png", cv.CV_LOAD_IMAGE_COLOR),
          cv.LoadImageM("intensity9.png", cv.CV_LOAD_IMAGE_COLOR),
          cv.LoadImageM("intensity10.png", cv.CV_LOAD_IMAGE_COLOR),
          cv.LoadImageM("intensity11.png", cv.CV_LOAD_IMAGE_COLOR)]
      

if 1:
    for i,img1 in enumerate(images1):
        #cv.SaveImage("final%04d.jpg" % i, img)
        cv.ShowImage("snap", img1)
        print "img= ",img1

if 1:
    for i,img2 in enumerate(images2):
        #cv.SaveImage("final%04d.jpg" % i, img)
        cv.ShowImage("snap", img2)
        print "img= ",img2
        
size = cv.GetSize(images1[0])
print "size= ", size
corners1 = [get_corners(i) for i in images1]
corners2 = [get_corners(i) for i in images2]

goodcorners1 = [co for (im1, (ok, co)) in zip(images1, corners1) if ok]
goodcorners2 = [co for (im2, (ok, co)) in zip(images2, corners2) if ok]
...
if True:
    ...
    print "initial error", silly(x1, goodcorners1)
    xopt = fmin(silly, x1, args=(goodcorners1,))
    print "initial error", silly(x1, goodcorners2)
    xopt = fmin(silly, x2, args=(goodcorners2,))
    print "xopt", xopt
    print "final error", silly(xopt, goodcorners1)
    print "final error", silly(xopt, goodcorners2)
    ...
    for co in goodcorners1:
        scrib1 = cv.CreateMat(480, 640, cv.CV_8UC3)
        cv.SetZero(scrib1)
        cv.DrawChessboardCorners(scrib1, (num_x_ints, num_y_ints), [xf(pt, poly) for pt in co], True)
        #cv.ShowImage("snap", scrib1)
        #cv.WaitKey()
    
    for co in goodcorners2:
        scrib2 = cv.CreateMat(480, 640, cv.CV_8UC3)
        cv.SetZero(scrib2)
        cv.DrawChessboardCorners(scrib2, (num_x_ints, num_y_ints), [xf(pt, poly) for pt in co], True)
        #cv.ShowImage("snap", scrib2)
        #cv.WaitKey()
    #sys.exit(0)
'''
for (i, (img, (ok, co))) in enumerate(zip(images, corners)):
    ...
    cv.WaitKey()
'''
#Setup CalibrateCamera2 augment

#Setup CalibrateStereo augment    
print len(goodcorners1)
print len(goodcorners2)
ipts1 = mk_image_points(goodcorners1)
ipts2 = mk_image_points(goodcorners2)
#print "ipts1= ",list(cvmat_iterator(ipts1))
#print "ipts2= ",list(cvmat_iterator(ipts2))
opts = mk_object_points(len(goodcorners1), .1)
npts = mk_point_counts(len(goodcorners1))
...
distortionRGB = cv.CreateMat(4, 1, cv.CV_64FC1)
distortionIR = cv.CreateMat(4, 1, cv.CV_64FC1)
...
'''
=======================================================================
Kinect RGB camera (640x480) Final
ChessBoard picture from RGBdemo
D = [0.22464481251757576, -0.47968370787671893, 0.0, 0.0]
K = [520.97092069697146,     0.0,                 318.40565581396697, 
     0.0,                     517.85544366622719, 263.46756370601804, 
     0.0,                     0.0,                 1.0]
=======================================================================

#distortion1 = [0.22464481251757576, -0.47968370787671893, 0.0, 0.0]
intrinsics1 = [(520.97092069697146,0.0,318.40565581396697),
               (0.0,517.85544366622719, 263.46756370601804),
               (0.0,0.0,1.0)]

=======================================================================
Kinect IR camera (640x480) Final 
ChessBoard picture from RGBdemo
D = [-0.1273506872313161, 0.36672476189160591, 0.0, 0.0]
K = [588.51686020601733,     0.0,                 320.22664144213843, 
     0.0,                     584.73028132692866, 241.98395817513071, 
     0.0,                     0.0,                 1.0]     
=======================================================================

#distortion2 = [-0.1273506872313161, 0.36672476189160591, 0.0, 0.0]
#intrinsics2 = [(588.51686020601733,0.0,320.22664144213843),(0.0,584.73028132692866,241.98395817513071),(0.0,0.0,1.0)] 
'''

exR1 = cv.CreateMat(len(goodcorners1), 3, cv.CV_32FC1)
exT1 = cv.CreateMat(len(goodcorners1), 3, cv.CV_32FC1)
exR2 = cv.CreateMat(len(goodcorners2), 3, cv.CV_32FC1)
exT2 = cv.CreateMat(len(goodcorners2), 3, cv.CV_32FC1)
         
'''Watch out the row and col of T''' 
...
cv.InitUndistortMap(intrinsics1, distortion1, mapx, mapy)
for img in images1:
    r = cv.CloneMat(img)
